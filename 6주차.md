...
> 상속은 슈퍼클래스의 기능을 이용하거나 **확장하기 위해서 사용되고**, 다중 상속의 모호성 때문에 하나만 상속 받을 수 있다. 부모의 기능을 받아서 확장하는데 많이 쓴다.(어미고양이 잔다, 새끼고양이 잔다{입양되서 잔다.})
> 인터페이스는 해당 인터페이스를 구현한 객체들에 대해서**동일한 동작을 약속하기 위해 존재한다.**  다형성을 위해 존재한다 볼 수 있다.(넓은 범위(생명체 먹다)를 정해주고 객체마다 다르게 구현 하면 된다.(인간 먹다, 코끼리 먹다, 고양이 먹다.)) 
### 다이나믹 메소드 디스패치 (Dynamic Method Dispatch)
